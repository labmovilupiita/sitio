...
class ReporteRoboDAO {
    ...
    function selectPostReporteRoboTablaDinamica() {
        ...
        $query = $con->prepare("SELECT r.idreporteRobo, r.idpublicacion, r.idclase FROM reporterobo r,publicacion p where r.idpublicacion=p.idpublicacion order by p.fecha desc");
        $query->execute();
        $con = NULL;
        while ($row = $query->fetch()) {
            $reporte = new ReporteRobo($row[0]);
