...
class UbicacionDAO {
    ...
    function totalUbicados() {
        $con = $this->dbc->conectar();
        //Si la conexion no fue exitosa
        if ($con == NULL) {
            return NULL;
        }
        $query = $con->prepare("SELECT count(idubicacion) as total from ubicacion where ubicado = 1;");
        $query->execute();
        $con = NULL;
        $row = $query->fetch();
        return $row['total'];
    ...
    function totalNoUbicados() {
        $con = $this->dbc->conectar();
        //Si la conexion no fue exitosa
        if ($con == NULL) {
            return NULL;
        }
        $query = $con->prepare("SELECT count(idubicacion) as total from ubicacion where ubicado = 2;");
        $query->execute();
        $con = NULL;
        $row = $query->fetch();
        return $row['total'];
