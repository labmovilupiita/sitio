...
<?php
	...
	function consultarTT(){
		...
		switch ($option) {
			case 1:
				$sql="SELECT id, titulo, fechainicial, fechafinal, totalreportes FROM tt WHERE id_profesor = ".$_COOKIE['userid']." AND tt.titulo ='".$str."' GROUP BY tt.id;";
				...
				break;
			case 2:
				$sql="SELECT id_tt FROM alumno WHERE (nombre = '".$str."' OR apellidopaterno = '".$str."' OR apellidomaterno = '".$str."') AND alumno.id_profesor = ".$_COOKIE['userid'];
				$result = mysqli_query($con,$sql);

				while($row = mysqli_fetch_array($result))
				  {

				  $sql1="SELECT id, titulo, fechainicial, fechafinal, totalreportes FROM tt WHERE id = '".$row['id_tt']."'";
				  $result1 = mysqli_query($con,$sql1);
				  ...
				  while($row1 = mysqli_fetch_array($result1)){
					  ...
					  $sql2="SELECT nombre, apellidopaterno, apellidomaterno, correo FROM alumno WHERE id_tt = ".$row['id_tt'];
					  $result2 = mysqli_query($con,$sql2);
					  echo "<td>";
					  while($row2 = mysqli_fetch_array($result2)){
					  	echo $row2['nombre']." ".$row2['apellidopaterno']." ".$row2['apellidomaterno']." [". $row2['correo']."]<br>";
	...
	function consultarAlumno(){
		...
		switch ($option) {
			case 1:
				$sql="SELECT * FROM alumno WHERE (nombre = '".$str."' OR apellidopaterno = '".$str."' OR apellidomaterno = '".$str."') AND alumno.id_profesor=".$_COOKIE['userid'];
				$result = mysqli_query($con,$sql);
				break;
			case 2:
				$sql="SELECT * FROM alumno WHERE correo = '".$str."' AND alumno.id_profesor=".$_COOKIE['userid'];
				$result = mysqli_query($con,$sql);
				break;
			case 3:
				$sql1="SELECT id FROM tt WHERE titulo = '".$str."' AND id_profesor='".$_COOKIE['userid']."';";
				$result1 = mysqli_query($con,$sql1);
